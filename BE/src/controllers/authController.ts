...
export const requestPasswordReset = async (
  ...
  try {
    ...
    console.log(
      `Email sent. Temporary password for ${email}: ${tempPassword}`
    );
    res.json({
...
export const changePassword = async (
  ...
  try {
    ...
    if (newPassword.length < 8) {
      res
        .status(400)
        .json({ message: "The new password must contain at least 8 characters." });
      return;
    }
    ...
    if (newPassword === currentPassword) {
      res
        .status(400)
        .json({ message: "The new password must be different from the current one." });
      return;
    }
