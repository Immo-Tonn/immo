...
export const setupDatabaseIndexes = async (): Promise<void> => {
  try {
    ...
    console.log(
      '✅ Composite index for images.realEstateObject + type created',
    );
...
export const listExistingIndexes = async (): Promise<void> => {
  try {
    ...
    console.log(
      'Images collection:',
      imagesIndexes.map(idx => idx.name),
    );

    const objectsIndexes = await RealEstateObjectsModel.collection
      .listIndexes()
      .toArray();
    console.log(
      'RealEstateObjects collection:',
      objectsIndexes.map(idx => idx.name),
    );
  } catch (error) {
